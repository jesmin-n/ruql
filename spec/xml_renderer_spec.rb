...
describe XMLRenderer do
  ...
  describe 'calling renderers' do
    ...
    it 'should call MultipleChoice renderer for SelectMultiple question' do
      ...
      expect(@x).to receive(:render_multiple_choice).with(q)
      @x.render(q)
    end
  end
  describe 'answer for radiobutton question' do
    ...
    ['n1','n2'].each do |wrong|
      it { is_expected.to have_xml_element(
  ...
  describe 'rendering' do
    describe 'correct answer without explanation' do
      ...
      it { is_expected.to have_xml_element('option').with_attribute('selected-score', '1') }
      it { is_expected.to have_xml_element('option/text', :value => c('correct')) }
      it { is_expected.not_to have_xml_element 'option/explanation' }
    end
    describe 'distractor with explanation' do
      ...
      it { is_expected.to have_xml_element 'option/explanation', :value => c('why') }
    end
    describe 'multiple choice question with answers' do
      ...
      it { is_expected.to have_xml_element('question/data/text') }
      it { is_expected.to have_xml_element('//option_group').containing(2, 'option') }
      it { is_expected.to have_xml_element('//option/text', :value => c('wrong')) }
    end    
  end
end
